...
// SDL GPU ABI; compile each stage separately via cmake/Shaders.cmake.
#ifdef PAPER_SPIRV
...
struct ShadowOutput { float4 position : SV_Position; float2 uv : TEXCOORD0; float depth : TEXCOORD1; };
struct ScreenOutput { float4 position : SV_Position; float2 uv : TEXCOORD0; };
...
struct ParticleOutput { float4 position : SV_Position; float2 uv : TEXCOORD0; float4 color : TEXCOORD1; };
#if defined(WORLD_VERTEX) || defined(SHADOW_VERTEX)
cbuffer FrameBuffer : register(b0, space1) { Frame frame; };
cbuffer ModelBuffer : register(b1, space1) { Model model; };
...
#endif
#elif defined(WORLD_FRAGMENT)
cbuffer FrameBuffer : register(b0, space3) { Frame frame; };
cbuffer MaterialBuffer : register(b1, space3) { Material material; };
COMBINED Texture2D<float4> baseTexture : register(t0, space2);
COMBINED Texture2D<float4> alternateTexture : register(t1, space2);
COMBINED Texture2D<float> shadowMap : register(t2, space2);
COMBINED SamplerState baseSampler : register(s0, space2);
COMBINED SamplerState alternateSampler : register(s1, space2);
COMBINED SamplerComparisonState shadowSampler : register(s2, space2);
float4 world_fragment(WorldOutput input) : SV_Target0 {
    float depth = dot(input.worldPosition - frame.cameraPosition.xyz, frame.cameraForward.xyz);
    if (depth < frame.cameraUp.w || depth > frame.cameraForward.w) discard;
    float2 uv = frac(input.uv);
    float4 color = baseTexture.Sample(baseSampler, uv);
    if (color.a < frame.fogColor.w) discard;
    if (material.flags.y > 0 && frame.effects.x > 0) {
        ...
        if (other.a >= frame.fogColor.w)
            color.rgb = lerp(color.rgb, other.rgb, mask * frame.effects.x);
    }
                ...
                if (all(shadowUV >= 0) && all(shadowUV <= 1)) {
                    ...
                    for (int y = -radius; y <= radius; ++y)
                        for (int x = -radius; x <= radius; ++x) {
                            ...
                            visibility += shadowMap.SampleCmpLevelZero(shadowSampler, tap, projected.z - bias);
                        }
...
#elif defined(SHADOW_FRAGMENT)
cbuffer FrameBuffer : register(b0, space3) { Frame frame; };
COMBINED Texture2D<float4> baseTexture : register(t0, space2);
COMBINED SamplerState baseSampler : register(s0, space2);
void shadow_fragment(ShadowOutput input) {
    if (input.depth < frame.shadowRight.w || input.depth > frame.shadowUp.w) discard;
    // Only the original alpha defines the silhouette, exactly as in the color pass.
    if (baseTexture.Sample(baseSampler, frac(input.uv)).a < frame.fogColor.w) discard;
}
#elif defined(PARTICLE_VERTEX)
cbuffer FrameBuffer : register(b0, space1) { Frame frame; };
cbuffer ParticleBuffer : register(b1, space1) { ParticleData particle; };
ParticleOutput particle_vertex(uint id : SV_VertexID) {
    ...
    float fog = clamp((depth - frame.fog.x) / frame.fog.y, 0.0, frame.fog.z);
    output.color = float4(lerp(particle.colorOpacity.rgb, frame.fogColor.rgb, fog) * shade, particle.colorOpacity.a * facing);
    return output;
}
#elif defined(PARTICLE_FRAGMENT)
float4 particle_fragment(ParticleOutput input) : SV_Target0 {
    float edge = saturate(1 - dot(input.uv, input.uv));
    return float4(input.color.rgb, input.color.a * edge * edge);
}
#elif defined(SCREEN_VERTEX)
ScreenOutput screen_vertex(uint id : SV_VertexID) {
    ScreenOutput output;
...
#elif defined(SCREEN_FRAGMENT)
cbuffer FrameBuffer : register(b0, space3) { Frame frame; };
COMBINED Texture2D<float4> sceneTexture : register(t0, space2);
COMBINED SamplerState sceneSampler : register(s0, space2);
float4 screen_fragment(ScreenOutput input) : SV_Target0 {
